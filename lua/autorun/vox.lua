...
local VoxListCmds = {'voxlist', 'voxhelp', 'voxmenu',}
...
local function DoPrefix(prefixes, cmd)
	for _, prefix in ipairs(prefixes) do
		if string.StartsWith(cmd, prefix) then return string.sub(cmd, #prefix + 1) end
	end
end
...
VOX_ADMINONLY = CreateConVar('vox_adminonly', 1, FCVAR_NOTIFY, 'Is the VOX Broadcaster admin only?')
VOX_DELAY = CreateConVar('vox_delay', 1, FCVAR_NOTIFY, 'Delay between VOX commands')
VOX_BUTTONS_ARE_SV = CreateConVar('vox_buttons_are_serversided', 0, FCVAR_NOTIFY, '')
if SERVER then
	...
	local function VoxSvCmd(ply, cmd, args, isButton)
		...
		if isButton or not VOX_NEXTBROADCAST or VOX_NEXTBROADCAST < CurTime() then
	...
	function CanBroadCast(ply)
		if VOX_ADMINONLY:GetBool() then return ply:IsAdmin() end
		return true
	end
	...
	net.Receive(NETSTRS.ListButton, function(length, ply)
		...
		if VOX_BUTTONS_ARE_SV:GetBool() and CanBroadCast(ply) then VoxSvCmd(ply, 'vox', str:Split(Delimiter)) end
	end)
else -- CLIENT
	VOX_PITCH = 100
	VOX_LEVEL = 70
	...
	function voxBroadcast(str, originalSender, entity, sndData)
		...
		local tbl = string.Explode(Delimiter, str)
